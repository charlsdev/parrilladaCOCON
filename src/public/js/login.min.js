...
if(document.querySelector('.contenedor-slider')){
   ...
   for (let y = 0; y < dots.children.length; y++) {
      //por cada dot que ecuentre le agregamos el evento click y ejecutamos la funcion de slide
...
for (let i = 0; i < mostrarClave.length; i++) {
   mostrarClave[i].addEventListener('click',()=>{
      if(inputPass[i].type==='password'){
         ...
         mostrarClave[i].classList.remove('bx-show-alt');

         //agregamos el nuevo icono
         mostrarClave[i].classList.add('bx-hide');
         ...
         //removemos la clase del icono
         mostrarClave[i].classList.remove('bx-hide');

         //agregamos el nuevo icono
         mostrarClave[i].classList.add('bx-show-alt');
...
if(document.getElementById('btnLogin')){
   ...
   btnLogin.addEventListener('click',(e)=>{
      ...
      if(username=='' || username==null){
         mostrarError('Por favor ingrese su cédula',msError);
